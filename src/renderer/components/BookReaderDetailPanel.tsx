          ...
          <div className="flex flex-col gap-5">
            <div>
              ...
              {[
                ...
                <div key={row.label} className="flex items-center py-2.5 border-b border-white/5 last:border-b-0">
                  ...
                  <span className="text-[13px] text-gray-300 truncate">{row.value || '—'}</span>
                </div>
              ))}
            </div>
            {book.description && (
              <div>
                <h3 className="text-[11px] font-bold uppercase tracking-wider text-gray-500 mb-2">Description</h3>
                <p className="text-[13px] leading-[1.6] text-gray-400 line-clamp-6">{book.description}</p>
              </div>
            )}
          </div>
        )}
                ...
                {sortedHighlights.map((hl) => (
                  ...
                  >
                    ...
                    <div className="flex-1 p-3 min-w-0">
                      ...
                      <span className="mt-2 block text-[10px] text-gray-600">{formatRelativeTime(hl.createdAt)}</span>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); onDeleteHighlight(hl.id); }}
                      className="absolute top-2 right-2 p-1.5 rounded opacity-0 group-hover:opacity-100 hover:bg-white/10 transition-all cursor-pointer text-gray-500 hover:text-red-400"
                    >
